...
get_org_repos() {
	org=$1
	curl -H "Authorization: token $GITHUB_TOKEN" -H "Accept: application/vnd.github.v3+json"  https://api.github.com/orgs/$org/repos 2>/dev/null | jq 'map(select(.[].url | .[].archived == false))' -r
}
